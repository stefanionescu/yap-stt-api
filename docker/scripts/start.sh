...
inject_api_key_into_config() {
    local api_key_value="${KYUTAI_API_KEY:-}"
    if [ -n "${api_key_value}" ]; then
        echo "[start] Injecting KYUTAI_API_KEY into authorized_ids"
        awk -v key="${api_key_value}" '
            BEGIN { replaced = 0 }
...
build_launch_command() {
    # Produces global LAUNCH_CMD that honors runtime-injected ${YAP_CONFIG}
    local -a args=("$@")
    LAUNCH_CMD="$*"
